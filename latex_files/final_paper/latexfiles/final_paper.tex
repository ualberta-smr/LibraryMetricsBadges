...
\cite{transparency,metrics}. Users may also dig up other library characteristics like functionality, usability,
quality, compatability, security to judge libraries with\cite{librarytrends,metrics,githubbadges}. 
These aspects often contribute to the popularity of a library
\cite{apiwave, librarytrends} that users find the most useful feature across different domains \cite{empiricalmetrics}.
Users depend on these factors i.e metrics that the related works above mention to select a library to use. 
...
There exists other related work that has focused on users decisions on libaries. 
Mora et al. 
