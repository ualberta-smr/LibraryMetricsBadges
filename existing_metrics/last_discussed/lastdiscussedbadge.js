...
module.exports = async (req,res) => {
    ...
    return new Promise(async (resolve, reject) => {
        let tagResponse = await axios.get(`${rootURL}/tags/${libName}/info?pagesize=10&order=desc&sort=popular&${SO}&apikey=${process.env.SO_KEY}`);
        ...
        let recentQuestion = await axios.get(`${rootURL}/questions?pagesize=1&order=desc&sort=creation&${SO}&tagged=${popularTagName}&apikey=${process.env.SO_KEY}`);
